<script lang="ts">
  ...
  export let colors: string[] = ["#DC4632","rgba(220,70,50,0.75)","#f1a090","#e0604c"];
  export let outerRadius: number = 70;
  export let tilt: number = 20;
  export let tiltSideway: number = 160;
  export let trail: number = 50;
  export let orbitSpeed: number = 4;
  export let pullSpeed: number = 0;
  ...
  type Particle = {
    ...
    speedOffset: number; colorIdx: number;
  };
  ...
  function initBlackhole() {
    ...
    const resize = () => {
      ...
      const d = dpr();
      bgCanvas.width = width * d; bgCanvas.height = height * d;
      bgCanvas.style.width = `${width}px`; bgCanvas.style.height = `${height}px`;
      fgCanvas.width = width * d; fgCanvas.height = height * d;
      fgCanvas.style.width = `${width}px`; fgCanvas.style.height = `${height}px`;
      sizeRef.w = width; sizeRef.h = height;
    };
    ...
    const pSize = 0.5 + (Math.max(1,Math.min(50,particleSize))-1)*(4/49);
    ...
    const initPts = (w: number) => {
      ...
      for (let i=0; i<particleCount; i++) {
        const r = voidRadius + Math.pow(Math.random(),2)*(outerR-voidRadius);
        pts.push({ angle:Math.random()*Math.PI*2, radius:r, height:(Math.random()-0.5)*16, speedOffset:0.75+Math.random()*0.5, colorIdx:Math.floor(Math.random()*colors.length) });
      }
    };
    initPts(sizeRef.w);
    ...
    const draw = (now: number) => {
      ...
      const outerR = outerRadFromSize(w);
      const voidCx=(voidX/100)*w, voidCy=(voidY/100)*h;
      const tiltRad=(tilt*Math.PI)/180, tiltSwayRad=(tiltSideway*Math.PI)/180;

      type PPt = {x:number;y:number;size:number;alpha:number;z:number;color:string;};
      const bg: PPt[]=[], fg: PPt[]=[];
      ...
      for (let i=0;i<pts.length;i++){
        ...
        if (pt.radius<voidRadius){
          pt.radius=voidRadius+0.7*(outerR-voidRadius)+Math.random()*0.3*(outerR-voidRadius);
          pt.angle=Math.random()*Math.PI*2;
          pt.height=(Math.random()-0.5)*16;
          continue;
        }
        const cosA=Math.cos(pt.angle),sinA=Math.sin(pt.angle);
        const xb=pt.radius*cosA,yb=pt.height,zb=pt.radius*sinA;
        const x1=xb,y1=yb*Math.cos(tiltRad)+zb*Math.sin(tiltRad),z1=-yb*Math.sin(tiltRad)+zb*Math.cos(tiltRad);
        ...
        const px=voidCx+x3d*sca,py=voidCy+y3d*sca;
      ...
      for (const p of bg) { ctx.globalAlpha=p.alpha;ctx.fillStyle=p.color;ctx.beginPath();ctx.arc(p.x,p.y,p.size,0,Math.PI*2);ctx.fill(); }
      ctx.globalAlpha=1;

      // Event horizon center
      if (showCenter) {
        const sphereGrad=ctx.createRadialGradient(voidCx-voidRadius*0.25,voidCy-voidRadius*0.3,voidRadius*0.05,voidCx,voidCy,voidRadius);
        sphereGrad.addColorStop(0,"rgba(10,5,2,1)");
        sphereGrad.addColorStop(0.65,"rgba(4,2,1,1)");
        sphereGrad.addColorStop(0.92,"rgba(18,8,5,1)");
        sphereGrad.addColorStop(1,"rgba(18,8,5,0.9)");
        ...
        const rimGrad=ctx.createRadialGradient(voidCx,voidCy,voidRadius*0.88,voidCx,voidCy,voidRadius*1.12);
        rimGrad.addColorStop(0,"rgba(220,70,50,0)");
        rimGrad.addColorStop(0.5,"rgba(220,70,50,0.18)");
        rimGrad.addColorStop(0.85,"rgba(220,70,50,0.28)");
        rimGrad.addColorStop(1,"rgba(220,70,50,0)");
        ctx.fillStyle=rimGrad;ctx.beginPath();ctx.arc(voidCx,voidCy,voidRadius*1.12,0,Math.PI*2);ctx.fill();
      }

      for (const p of fg) { fgCtx.globalAlpha=p.alpha;fgCtx.fillStyle=p.color;fgCtx.beginPath();fgCtx.arc(p.x,p.y,p.size,0,Math.PI*2);fgCtx.fill(); }
      fgCtx.globalAlpha=1;
    ...
    const io=new IntersectionObserver(([e])=>{
      if (!e.isIntersecting) { cancelAnimationFrame(animRef); }
      else { lastTime=performance.now(); animRef=requestAnimationFrame(draw); }
    },{threshold:0});
